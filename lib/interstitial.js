...
/**
 * @param {string} token Signed proof that this page was actually served. `/go` and
 *   `/adview` reject requests without it, so the countdown can't be skipped by hand.
 */
export function renderInterstitial(link, token) {
  const slug = encodeURIComponent(link.slug);
  const t = encodeURIComponent(token);
  const goUrl = `/${slug}/go?t=${t}`;
  const adviewUrl = `/${slug}/adview?t=${t}`;
  ...
  // Emitted only when there is a script ad — otherwise the page must not reference
  // /adview at all.
  const adviewJs = link.adScript
    ? `setTimeout(function () {
    navigator.sendBeacon && navigator.sendBeacon(${JSON.stringify(adviewUrl)});
  }, 2500);`
    : '';
